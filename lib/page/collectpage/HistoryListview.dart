...
import 'package:flutter_app/page/item/HistoryArticleItem.dart';
...
class FavorateList extends State<HistoryListview> with AutomaticKeepAliveClientMixin {
  ...
  void getDataAsync(int pn){
    DBOperationArticle.queryByPageno(pn).then((datas){
      if(pn==0){
        pageno=0;
        data.clear();
      }
      if(isValidList(datas)){
        data.addAll(datas);
        pageno++;
      }
              ...
              child: Align(
                ...
                child: FreshContainer(
                    child:ListView(
                      ...
                      children: data.map((p) =>
                          HistoryArticleItem(checkObject.contains(p), p, _clickItem,_removeCallback)
                      ).toList(),
                    ),
